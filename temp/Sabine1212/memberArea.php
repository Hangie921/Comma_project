...
<!-- extUpload開始 -->
	<section id="extUpload" data-section-name="extUpload">
		...
		<div class="mbox">
			...
			$sql = "select orderNo,status,member					 
					from commaorder
					where member=:member and status=2
					order by orderNo desc";
					
			$exh = $pdo->prepare( $sql );
			$exh -> bindParam(":member",$_SESSION["member"]);
			$exh -> execute();
			while( $exhRow = $exh->fetch(PDO::FETCH_ASSOC)){  
				...
				<div><div class="exbText">展覽開始日期 /</div> <input type="date" name="startDate" id="startDate">
				</div>
	...
	<div id="lightboxExtUpload">
		...
		<div class="mbox boxLB">
			<span class="exbTitle">展覽資訊上傳</span>
				...
				<div class="exbConLB"><div class="exbTexta">展覽結束日期 /</div><div class="exbTextLB"  id="exhEndDate">2015-11-04</div>				
				</div>

				<div class="imgLB"><img src="img/exb-5.jpg" ></div>
				
				<div class="exbConLB"><div class="exbTexta">展覽名稱 /</div><div class="exbTextLB"  id="exhExhiName">日本人的襪子</div>
				</div>
								
				
				<div class="bb">
					<div class="exbTexta">展覽說明 / </div>
					<div class="exbTextLB aa" id="exhInstro">
					日本人從早到晚一直都是忙碌的生活，騎腳踏車、吃早餐、在外遛躂、搭電車，然後去上班，對他們而言，這就是一天，雖然每天平凡無奇，但他們每天都過得十分充實
					</div>
				</div>
...
<!-- 诶咖系將展覽轉到lightbox -->
<script>
	function exhiToLightBox(){
		/*alert($('#ExhGet select').val());*/
		/*$('#exhiName').appendTo('#exhExhiName');*/
		$("#submitMod").click(function(){
    $('#exhiName').innerHTML('#exhExhiName');
  });
	}
</script>
</body>
