...
ob_start();
session_start();
$_SESSION["memName"] = "八神太一";
$_SESSION["phone"] = "0933689121";
$_SESSION["memEmail"] = "couldbedon@gmail.com";
$_SESSION["siteNo"] = 1;
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/component2_480.css" media="screen and (max-width: 480px)">
		...
		<div class="control_menu">
			...
			<nav class="nav-circleslide">
				<a class="prev" href="javascript:void(0);">
					<span class="icon-wrap"><svg class="icon" width="32" height="32" viewBox="0 0 64 64"><path id="arrow-left-1" d="M46.077 55.738c0.858 0.867 0.858 2.266 0 3.133s-2.243 0.867-3.101 0l-25.056-25.302c-0.858-0.867-0.858-2.269 0-3.133l25.056-25.306c0.858-0.867 2.243-0.867 3.101 0s0.858 2.266 0 3.133l-22.848 23.738 22.848 23.738z" /><use xlink:href="#arrow-left-1"></svg></span>
					<div><img src="img/2.jpg" alt="Previous thumb"/></div>
				</a>
				<a class="next" href="javascript:void(0);">
					<span class="icon-wrap"><svg class="icon" width="32" height="32" viewBox="0 0 64 64"><path id="arrow-right-1" d="M17.919 55.738c-0.858 0.867-0.858 2.266 0 3.133s2.243 0.867 3.101 0l25.056-25.302c0.858-0.867 0.858-2.269 0-3.133l-25.056-25.306c-0.858-0.867-2.243-0.867-3.101 0s-0.858 2.266 0 3.133l22.848 23.738-22.848 23.738z" /><use xlink:href="#arrow-right-1"></svg></span>
					<div><img src="img/1.jpg" alt="Next thumb"/></div>
				</a>
			</nav>		
		</div>		
	</div>
	...
	<div class="leftbox">
	 ...
	 viewBox="2326.7 -1090.1 1754.3 2955.3" style="enable-background:new 2326.7 -1090.1 1754.3 2955.3;" xml:space="preserve">
			...
			<div class="content">				
				...
				<span class="maxmen">容納人數 : 30人</span>
				<span class="address">地點：台北市萬華區一段435號</span><br/>
...
//$sql = "select (memName, memEmail, phone) from member where memName=:memName, memEmail=:memEmail and phone=:phone";
//$statement = $pdo -> prepare($sql);
//$statement -> bindValue(":memName", $_SESSION["memName"]);
//$statement -> bindValue(":memEmail", $_SESSION["memEmail"]);
//$statement -> bindValue(":phone", $_SESSION["phone"]);
//$statement -> execute();
...
?>-->
	...
	<div class="orderbox-container">	
		<div class="close"><img src="img/close.png" height="36"></div>
		<h3 id="roomname-order">荒野屋<input type="hidden" value="<?php echo $_SESSION['siteNo']?>"/></h3>
		<div class="orderbox">
			<h3 id="memName"><?php echo $_SESSION["memName"]?></h3>
	<form action="space-add.php" method="post">	
	<input type="hidden" name="totalamount" id="total2">	
			<div class="info topinfo">
				<ul>
					<li><span>聯絡電話 / </span><span id="tel" name="tel"><?php echo $_SESSION["phone"]?></span></li>
					<li><span>電子信箱 / </span><span class="fillin" id="email"><?php echo $_SESSION["memEmail"]?></span></li>					
					<li><span>*出租時間 / </span><input type="date" id="startTime" name="startTime" placeholder="2015/11/07"></li>
					<li><span>*結束時間 / </span><input type="date" id="endTime" name="endTime" placeholder="2015/11/20"></li>				
					<li><span>價格 / </span><span id="subtotal"><?php echo $_SESSION["price"]?></span>NT</li>			
					<!-- <li><span>折扣優惠 / </span><span name="coda" id="coda"><?php echo $_SESSION["discount"]?></span> -->
					<!-- </li> -->
				</ul>
<!--<?php
//	}
...
?>-->
				<div class="agreecontainer">
					...
					<div class="agreecontent">威秀影城網站係依據本項條款提供網站中各項會員服務 (以下簡稱「本服務」)。當您使用本服務時，即表示您已全部閱讀且瞭解並同意接受本約定書之所有內容。方得使用或繼續使用本服務。當您使用或繼續使用本服務時，即推定您的家長（或監護人）已全部閱讀且瞭解並同意接受本約定書之所有內容及其後修改變更。當您使用或繼續使用本服務時，即推定您的家長（或監護人）已全部閱讀且瞭解並同意接受本約定書之所有內容及其後修改變更。</div>
				</div>
